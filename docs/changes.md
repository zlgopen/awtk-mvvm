...
2026/09/18
  ...
  * 完善 binding_context_update_to_view 的代码结构。（感谢朝泽提供补丁）

2026/09/10
  * 修复 UI 文件中未设置 v-model 时 ui_loader_mvvm 加载结果有误的问题。（感谢朝泽提供补丁）
  * 修复 ui_loader_mvvm 全局使用时上下文数据会被污染的问题。（感谢朝泽提供补丁）
  * 修复诸如 tab_button 等动态加载 UI 时加载结果有误的问题。（感谢朝泽提供补丁）

2026/09/08
  * 新增 data_binding 等支持 clone。 （感谢朝泽提供补丁）

2026/08/28
  * 新增 custom binder 的名称支持通配符（*）。(感谢福明提供补丁)
  * 新增 data_binding_set_prop_cbs，支持在更新数据时进行额外的处理。(感谢福明提供补丁)
...
2026/03/04
  * 过滤属性值为空的MVVM绑定设置，减少不必要的打印（感谢朝泽提供补丁）

2025/10/16
  * 完善文档(感谢俊杰提供补丁)

2025/10/15
  * 完善打开窗体的导航请求 （感谢兆坤提供补丁)
