...
static ret_t view_model_dummy_on_destroy(object_t* obj) {
...
static int32_t view_model_dummy_compare(object_t* obj, object_t* other) {
  return tk_str_cmp(obj->name, other->name);
}

static ret_t view_model_dummy_set_prop(object_t* obj, const char* name, const value_t* v) {
  view_model_dummy_t* dummy = VIEW_MODEL_DUMMY(obj);
...
static ret_t view_model_dummy_get_prop(object_t* obj, const char* name, value_t* v) {
  view_model_dummy_t* dummy = VIEW_MODEL_DUMMY(obj);
  return_value_if_fail(obj != NULL && name != NULL && v != NULL, RET_BAD_PARAMS);
