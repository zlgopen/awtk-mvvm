...
view_model_t* book_store_view_model_attach(object_t* obj, uint32_t index) {
  ...
  book_t* book = book_store_get(book_store_view_model->book_store, index);
...
static ret_t book_store_view_model_set_prop(object_t* obj, const char* name, const value_t* v) {
  ...
  book_store_t* book_store = ((book_store_view_model_t*)(obj))->book_store;
...
static ret_t book_store_view_model_get_prop(object_t* obj, const char* name, value_t* v) {
  ...
  if (tk_str_eq("items", name)) {
     value_set_uint32(v, book_store_get_items(book_store));
     return RET_OK;
  }
...
static bool_t book_store_view_model_can_exec(object_t* obj, const char* name, const char* args) {
  ...
  book_store_t* book_store = vm->book_store;
  if (tk_str_eq("clear", name)) {
    return TRUE;

  } else if (tk_str_eq("remove", name)) {
    return book_store_can_remove(book_store, tk_atoi(args));
...
static ret_t book_store_view_model_exec(object_t* obj, const char* name, const char* args) {
  ...
  book_store_t* book_store = vm->book_store;
  if (tk_str_eq("clear", name)) {
    return book_store_clear(book_store);

  } else if (tk_str_eq("remove", name)) {
    return book_store_remove(book_store, tk_atoi(args));

  } else if (tk_str_eq("add", name)) {
    return book_store_add(book_store);
  }
...
static ret_t book_store_view_model_on_destroy(object_t* obj) {
  ...
  book_store_destroy(vm->book_store);
...
view_model_t* book_store_view_model_create_with(book_store_t* book_store) {
  ...
  book_store_view_model->book_store = book_store;
  ENSURE(book_store_view_model->book_store != NULL);
...
view_model_t* book_store_view_model_create(navigator_request_t* req) {
  return book_store_view_model_create_with(book_store_create());
}
